...
from .models import Mesa, Pedido, PedidoItem, Producto
        ...
        for prod in productos:
            ...
            if cantidad > 0:
                ...
                productos_a_agregar.append(
                    {"producto": prod, "cantidad": cantidad, "observacion": observacion}
                )
...
def cerrar_pedido(request, id):
    ...
    if request.method == "POST":
        ...
        if metodo_pago not in ["efectivo", "tarjeta", "transferencia"]:
            ...
            return redirect("cerrar_pedido", id=mesa.id)
...
def cancelar_pedido(request, id):
    ...
    return render(
        request,
        "pedidos/cancelar_pedido.html",
        {"mesa": mesa, "pedido_actual": pedido_actual, "items": items, "total": total},
    )
...
def eliminar_items(request, id):
    ...
    pedido_actual = Pedido.objects.filter(
        ...
        estado__in=[
            ...
            "en_preparacion",
            "listo",
        ],  # Consideramos estos estados como "activos"
        restaurante=restaurante,
        esta_pagado=False,
    ).first()
    ...
    return render(
        request,
        "pedidos/eliminar_items_a_pedido.html",
        {"mesa": mesa, "pedido_actual": pedido_actual, "items": items},
    )
...
def eliminar_item_especifico(request, id):
    ...
    if request.method == "POST":
        item = get_object_or_404(PedidoItem, id=id, pedido__restaurante=restaurante)
...
def cocina(request):
    ...
    pedidos_pendientes = Pedido.objects.filter(
        restaurante=restaurante, estado="en_preparacion"
    )
...
def enviar_cocina(request, pedido_id):
    ...
    if pedido.estado in ["pendiente", "listo", "en_preparacion", "aceptado"]:
        ...
        pedido.items.filter(enviado_a_cocina=False).update(
            enviado_a_cocina=True, numero_envio=pedido.total_envios
        )
...
def detalle_pedido_online(request, id):
    ...
    pedido = get_object_or_404(Pedido, id=id, restaurante=restaurante, origen="online")
    ...
    return render(
        request,
        "pedidos/detalle_pedido_online.html",
        {
            "pedido": pedido,
            "items": items,
            "total": total,
...
def aceptar_pedido_online(request, id):
    ...
    if request.method == "POST":
        if pedido.estado == "pendiente":
            ...
            messages.success(request, f"Pedido online #{pedido.id} enviado a cocina.")
    ...
    if request.method == "POST":
        ...
        if pedido.tipo_entrega == "delivery":
            ...
            if pedido.estado == "listo":
                # Si es transferencia y todavía no está pagada,
                # no permitimos enviar el pedido.
                if pedido.metodo_pago == "transferencia" and not pedido.esta_pagado:
                    messages.error(
                        request,
                        f"El pedido online #{pedido.id} "
                        f"tiene una transferencia pendiente de verificar.",
                    )
                ...
                messages.success(request, f"Pedido online #{pedido.id} va en camino.")

            # EN CAMINO → ENTREGADO
            elif pedido.estado == "en_camino":
                pedido.estado = "entregado"

                messages.success(request, f"Pedido online #{pedido.id} fue entregado.")
        ...
        # ========================================
            ...
            if pedido.estado == "listo":
                ...
                messages.success(
                    request, f"Pedido online #{pedido.id} marcado como entregado."
                )
...
def cobrar_pedido_online(request, id):
    ...
    pedido = get_object_or_404(
        Pedido, id=id, restaurante=restaurante, origen="online", esta_pagado=False
    )
    ...
    if pedido.estado not in ["listo", "entregado"]:
        messages.error(request, "Este pedido todavía no puede registrar un pago.")
    ...
    if restaurante.propina_activa:
        propina = total * restaurante.porcentaje_propina / 100
    ...
    if request.method == "POST":
        ...
        if metodo_pago not in ["efectivo", "tarjeta", "transferencia"]:
            ...
            return redirect("cobrar_pedido_online", id=pedido.id)
...
def marcar_pedido_pagado(request, id):
    ...
    if request.method == "POST":
        ...
        messages.success(
            request, f"Pago del pedido online #{pedido.id} verificado correctamente."
        )
...
def seguimiento_pedido(request, token):
    ...
    pedido = get_object_or_404(Pedido, token_seguimiento=token, origen="online")
